...
Warden is building a self-custody, smart-account browser-extension wallet by [drinkerlabs](https://drinkerlabs.info). The implemented on-chain foundation uses a passkey root (P-256), bounded session keys, and shared per-transaction, daily and 30-day spending limits. Timelocked larger moves and guardian recovery are planned Phase 1C work. The shipped development extension does not yet offer account setup, app connections, signing or sending.
...
| **Shipped extension** | MV3 worker/content/popup boundaries, encrypted keyring infrastructure and a request-bound review page. The development popup explains capability limits and supports connection retry. The review page displays full origins and separate decoding, simulation and policy evidence. **All wallet methods remain unavailable.** |
...
| **Next product milestones** | Local onboarding/account registry; an explicitly bound provider bootstrap; production ceremony and RPC composition; sending; Wallet Standard registration. Production KDF policy, extension identity/origin, real trust material and independent security review remain required. Services are not started. |
