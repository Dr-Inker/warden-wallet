...
Warden is building a self-custody, smart-account browser-extension wallet by [drinkerlabs](https://drinkerlabs.info). The implemented on-chain foundation uses a passkey root (P-256), bounded session keys, and shared per-transaction, daily and 30-day spending limits. Timelocked larger moves and guardian recovery are planned Phase 1C work. The development extension supports saving and selecting public account addresses locally. Wallet creation, balances, app connections, signing and sending remain unavailable.
...
| **Next product milestones** | Verify local public-account onboarding; passkey wallet creation with explicit identity/trust decisions; an explicitly bound provider bootstrap; production ceremony and RPC composition; sending; Wallet Standard registration. Production KDF policy, extension identity/origin, real trust material and independent security review remain required. Services are not started. |
