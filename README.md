...
| **Extension / services** | Extension trust-boundary and keyring foundation in progress: real MV3 worker/content/popup boundaries, trusted-only storage, canonical encrypted records, absolute unlock sessions, authenticated password activation, and local-only signer-seed leases exist. There is still no onboarding, account registry, approval UI, RPC/sign/send path, production KDF policy, or usable wallet method. Services are not started. |
