...
| **Extension / services** | Extension trust-boundary and keyring foundation in progress: real MV3 worker/content/popup boundaries, trusted-only storage, self-contained context-bound encrypted records, browser-owned runtime-origin checking, authenticated password and wake activation, absolute unlock sessions, and local-only signer-seed leases exist. There is still no onboarding/migration, account registry, approval UI, RPC/sign/send path, production KDF policy, or usable wallet method. Services are not started. |
