...
| **Extension / services** | Extension trust-boundary and keyring foundation in progress: real MV3 worker/content/popup boundaries, trusted-only storage, self-contained context-bound encrypted records, browser-owned runtime-origin checking, authenticated password and wake activation, absolute unlock sessions, local-only signer-seed leases, durable approval/replay substrates, bind-before-window composition, bounded page-Promise replay protection, a volatile exact-id/digest approval action, a closed durable signed-or-failed terminal outcome path, bounded content-side recovery, browser-tested replacement-Port/durable-restart routing, and one immutable cross-worker deadline/cancellation/receipt-settlement protocol exist internally. A temporary-extension Chromium contract composes that graph through the real approval UI and independently verifies that the exact reviewed message receives one authenticated signature and returns as the durable page result; it also caught and removed Node-only `Buffer` use from browser transaction preparation. Restart contracts now cut after durable signing, after signer-result production but before commit, inside the native strict IndexedDB completion transaction, after the page has accepted the exact terminal and emitted its receipt while background settlement is still incomplete, and after background receipt acceptance/lease completion immediately before the native final-settlement enqueue. A locked replacement must replay independently verified durable bytes or persist one `worker-restarted` failure; it may never retry signing, expose uncommitted bytes, duplicate page settlement, restore authority, or perform new RPC/selection work. This is deterministic test provenance, not a product path or a power-loss guarantee. All provider/coordinator/action/result/page/recovery owners remain excluded from production, whose approval page still reports signing unavailable. There is still no preparation/pending-approval, post-terminal-enqueue/pre-page-settlement, or native-settlement-enqueued-but-undelivered cut proof, onboarding/migration, account registry, non-empty production RPC/release, production-composed signing or provider delivery, send path, production KDF policy, Wallet Standard registration, or usable wallet method. Services are not started. |
