...
The command rebuilds the extension and writes five ignored, local artifacts
...
- `warden-extension-<version>.artifact.json`, which binds the clean source SHA,
  ...
  the complete ZIP hash, and the exact bytes of both evidence sidecars;
...
- `warden-extension-<version>.bundle-inputs.json`, a canonical record of every
  ...
  binds the clean source and ZIP hash.
...
upload ZIP and both evidence sidecars against an already reviewed artifact
manifest:
...
outputs only. The dependency sidecar still does **not** assert that every listed
package contributed bundle bytes, and the bundle sidecar does not cover copied
HTML, CSS, `manifest.json`, icons, build-tool/configuration/environment inputs,
or legal disposition. Neither sidecar proves that a declared license is legally
...
controls. Replacing the ZIP and all three unsigned, co-generated JSON files is not
