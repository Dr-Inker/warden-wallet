...
The command rebuilds the extension and writes six ignored, local artifacts
...
- `warden-extension-<version>.artifact.json`, which binds the clean source SHA,
  ...
  the complete ZIP hash, and the exact bytes of all three evidence sidecars;
...
- `warden-extension-<version>.static-inputs.json`, a canonical source/output
  ...
  `manifest.json`'s JSON parse/two-space/newline serialization.
...
upload ZIP and all three evidence sidecars against an already reviewed artifact
manifest:
...
node apps/extension/scripts/verify-release.mjs \
  ...
  /path/to/candidate.static-inputs.json
```
...
other files, and no sidecar covers build-tool/configuration/environment inputs
or legal disposition. No sidecar proves that a declared license is legally
...
controls. Replacing the ZIP and all four unsigned, co-generated JSON files is not
