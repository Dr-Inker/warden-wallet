# Archived NEXT-SESSION pickup memos (2026-08-19 → 2026-09-01)

Rotated out of `docs/NEXT-SESSION.md` on 2026-09-02 (Fable audit R-5: the live memo had grown to 993 KB / 121 stacked memos).
Verbatim, newest first. The memo that was on top at rotation time stays in the live file.
...
>
> Contract commit `9421f5892ee08d934a8bcd2502773e498126f14c`
> precedes behavioral RED commit
> `0dadc1ed8101ca57ea9907005c43b5df89e5aa29`. From that clean RED SHA, this
> exact command exited **1**:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && node --check apps/extension/test/verify-store-package-infozip.test.mjs && pnpm --filter @warden/extension exec vitest run test/verify-store-package-infozip.test.mjs -t "kills a stalled store unzip direct child within its archive deadline"
> ```
>
> Vitest ran the selected test, skipped two, and failed in **12.46 s** with all
> three missing behaviors measured together: the verifier succeeded rather than
> failing closed, the fake direct child wrote its completion marker, and elapsed
> time exceeded ten seconds. The fake wrote its start marker and the existing
> success cleanup removed the private directory. The explicit 20-second test
> deadline did not fire. This is the contracted behavioral RED, not a harness or
> syntax failure.
>
> Implementation/evidence commit
> `d53bea74d75228707c2d9df538afbe66c2a20866` calculates the contracted timeout
> from the already verified embedded archive length and passes it with
> `killSignal: "SIGKILL"` to the existing store Info-ZIP `execFile` call. README
> and release-integrity prose state the direct-child/non-process-group boundary.
>
> From that clean implementation SHA, this exact single-contract command exited
> **0** and printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && node --check apps/extension/scripts/verify-store-package.mjs && node --check apps/extension/test/verify-store-package-infozip.test.mjs && pnpm --filter @warden/extension exec vitest run test/verify-store-package-infozip.test.mjs && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-verify-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed **3/3** tests in **6.15 s**, syntax, selected cleanup, diff checks,
> and both clean-tree guards. The fake direct child wrote its start marker, was
> killed/fail-closed in about **6.0 s**, did not write its completion marker, and
> left no private directory. C74's minimal environment and C73/C72's successful
> descriptor and mutation-refusal probes remained green.
>
> From that same clean SHA, this exact focused/release command exited **0** and
> printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && node --check apps/extension/scripts/verify-store-package.mjs && node --check apps/extension/test/verify-store-package-infozip.test.mjs && pnpm --filter @warden/extension exec vitest run test/verify-store-package-infozip.test.mjs test/verify-store-package-cli.test.mjs test/store-package.test.mjs test/release-source-tag.test.mjs test/verify-release-source-tag-cli.test.mjs test/release-recipe-input-evidence.test.mjs && pnpm --filter @warden/extension typecheck && pnpm --filter @warden/extension release:gate && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-recipe-evidence-test-*' -o -name 'warden-release-unzip-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed **55/55** focused tests, typecheck, canonical verification of **8**
> payload files, **60** production/peer components, **4** JavaScript bundles,
> **101** positive bundle inputs, **4** static inputs, **25** exact recipe inputs,
> the canonical unpacked tree, upload Info-ZIP parsing, selected cleanup, diff
> checks, and both clean-tree guards.
>
> From that same clean SHA, this exact extension-wide command exited **0** and
> printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && pnpm --filter @warden/extension test && pnpm --filter @warden/extension typecheck && pnpm --filter @warden/extension build && if rg -n 'verifyEmbeddedArchiveWithInfoZip|temporary embedded archive|warden-store-package-verify|infoZipEnvironment|infoZipTimeoutMs|cwd: temporaryDirectory|release-cli-arguments|normalizeReleaseCliArguments|release-input-file|readBoundedRegularFile|O_NOFOLLOW|verifyArchiveWithInfoZip|assertTemporaryArchiveUnchanged|INFO_ZIP_ENVIRONMENT|INFO_ZIP_TIMEOUT|killSignal|LC_ALL|read-only seal|chmod\(0o400\)|verify-store-package|store-package|verify-release|artifactReview|reviewedUploadArchive|expectedStorePackageSha256|dualReleaseReport|OpenPGP verification|GIT_GPG_LAUNCHER' apps/extension/dist; then exit 1; fi && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' -o -name 'warden-release-tag-message-cli-test-*' -o -name 'warden-release-input-file-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-openpgp-signature-policy-test-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-reviewed-artifact-signature-*' -o -name 'warden-reviewed-artifact-signature-test-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed extension **600/600**, typecheck/build, emitted release-tooling
> exclusion (including the store timeout/kill policy), selected cleanup, diff
> checks, and both clean-tree guards. At this SHA the artifact, bundle, recipe,
> dependency, and static sidecar SHA-256 values were respectively
> `530a4c6a84e92a602e0a8b2d47ef2563f549b12b81221a73df2901bd08517d0a`,
> `c9bef59759de4b5b92efed51cb9250334637ae722402af252adc1f4790c75a53`,
> `7beee1098d23a5ad0099eb220f4c41fc6867d1098cc0ba818c2a6c21a189138b`,
> `54065e8defaf6bfc8ebd46d0a0c8f8a93a7ff692db26ac875dbb32da164024f9`,
> and `85bfe8c4c7104514e9ae0fd022705a837d722494844d98110e04f37b6fcf9784`.
> The recipe sidecar was **5,127 bytes** with 25 inputs. The store verifier was
> **10,346 bytes** at SHA-256
> `9cedf84cc3192d8ae95b5f6b7abbf58c539cd9f2e3e189c856ad92731205e166`.
...
> **25** recipe inputs, the store and upload verifiers' bounded/minimal-
> environment Info-ZIP parsing, the real sequential two-clean-checkout
> rehearsal, emitted release-tooling exclusion, selected temp-directory cleanup,
> diff checks, and both clean-tree guards. The Argon2 elapsed p50/p95 were
> **934.0/940.6 ms**, host-task delay p50/p95 were **63.5/68.9 ms**, and
> password-buffer wiping was true. The final rehearsal compared **14** files and
> produced a **3,810-byte** canonical report with SHA-256
> `f44bff1ce6b707283dd25c67a1a684e05d52570243d2d4a66ca0af3f80be14b2`.
> At the full-gated SHA the artifact, bundle, recipe, dependency, and static
> sidecar SHA-256 values were respectively
> `04b4a880d880f66f80e227df10eb4cda81330e0b9dd8be3746e2b63cc243458f`,
> `084c6ec506e948c2cd289eef714230712c8c91835b1beec2742b65e682fe9681`,
> `f11d278eab4ad415f67bb0152dcf08bd08ba0cba3cea1570785300c97f3b51d4`,
> `0668f70cc79355561d9ecb467a71f4ae2cf4f2c70d5e73c4a157b27e330b3ca5`,
> and `71d74cc47d8e87bdf14c08bc4afe16a9bf20daee80e748095cf8749084e2626b`.
> The recipe sidecar remained **5,127 bytes** with 25 inputs. The store verifier
> remained **10,346 bytes** at SHA-256
> `9cedf84cc3192d8ae95b5f6b7abbf58c539cd9f2e3e189c856ad92731205e166`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
...
> root defense, or process-group/descendant confinement. No invariant or
> production-release status moves.
...
>
> Implementation/evidence commit
> `21b7da8bbfe2c06983505f0a5c69304bf01d24eb` passes an explicit frozen object
> containing only call-time parent `PATH` (with the contracted absent-value
> fallback), `LANG=C`, and `LC_ALL=C` to the store Info-ZIP call. README and
> release-integrity prose state the cooperative, non-sandbox boundary.
>
> From that clean implementation SHA, this exact single-contract command exited
> **0** and printed the same SHA before and after:
>
...
> ```
>
> It passed **2/2** tests in **163 ms**, syntax, selected cleanup, diff checks,
> and both clean-tree guards. The success probe measured exactly the three
> contracted environment keys and values, absence of the unrelated marker and
> `TMPDIR`, and every C73 descriptor/digest/mode/replacement/working-directory/
> output predicate. The mutation probe still failed closed with empty cleanup.
>
> From that same clean SHA, this exact focused/release command exited **0** and
> printed the same SHA before and after:
>
...
> ```
>
> It passed **54/54** focused tests, typecheck, canonical verification of **8**
> payload files, **60** production/peer components, **4** JavaScript bundles,
> **101** positive bundle inputs, **4** static inputs, **25** exact recipe inputs,
> the canonical unpacked tree, upload Info-ZIP parsing, selected cleanup, diff
> checks, and both clean-tree guards.
...
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && pnpm --filter @warden/extension test && pnpm --filter @warden/extension typecheck && pnpm --filter @warden/extension build && if rg -n 'verifyEmbeddedArchiveWithInfoZip|temporary embedded archive|warden-store-package-verify|infoZipEnvironment|cwd: temporaryDirectory|release-cli-arguments|normalizeReleaseCliArguments|release-input-file|readBoundedRegularFile|O_NOFOLLOW|verifyArchiveWithInfoZip|assertTemporaryArchiveUnchanged|INFO_ZIP_ENVIRONMENT|INFO_ZIP_TIMEOUT|killSignal|LC_ALL|read-only seal|chmod\(0o400\)|verify-store-package|store-package|verify-release|artifactReview|reviewedUploadArchive|expectedStorePackageSha256|dualReleaseReport|OpenPGP verification|GIT_GPG_LAUNCHER' apps/extension/dist; then exit 1; fi && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' -o -name 'warden-release-tag-message-cli-test-*' -o -name 'warden-release-input-file-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-openpgp-signature-policy-test-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-reviewed-artifact-signature-*' -o -name 'warden-reviewed-artifact-signature-test-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed extension **599/599**, typecheck/build, emitted release-tooling
> exclusion (including the store environment policy), selected cleanup, diff
> checks, and both clean-tree guards. At this SHA the artifact, bundle, recipe,
> dependency, and static sidecar SHA-256 values were respectively
> `911953f85f28113c13cdac9817e5a46892834932c629d18a782743f429cf819d`,
> `2e49aed6e6ca206b21c424407b18426a255364dea6fef4d3fa068ce6db20ab3c`,
> `6dba51366dd6ad4e19923aec0d268a11c1244f3ed0c48ec59f4fb802252dcf17`,
> `567d6e0189f1460c8ebbc9f2ce829e93807711303e3f6dce83f2d682a6c32c57`,
> and `ef4d13e89b24fdea3c5ead2fd29d8fbbafc2d592f9664b13f62ec18bb897d411`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The store verifier
> was **9,906 bytes** at SHA-256
> `46075410f59d44645b887fb4afd4556ed13eda76a60f4cda3171715b1e980945`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
> Exact store grammar/signature/digest/output behavior, C73 private working
> directory, C72 stable-byte handoff, upload-verifier C63–C71 behavior, and
> provider fixed unavailable are unchanged.
>
> Evidence-ledger/full-gated commit
> `21f15aa5ea709685c60c18e80410f532fb0423fb` ran this exact FULL command from a
> clean tree. It exited **0** and printed that same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && env npm_config_cache=/tmp/warden-npm-cache bash .claude/test-gate.sh && env npm_config_cache=/tmp/warden-npm-cache pnpm --filter @warden/extension release:gate && env npm_config_cache=/tmp/warden-npm-cache pnpm --filter @warden/extension release:dual-local && if rg -n 'verifyEmbeddedArchiveWithInfoZip|temporary embedded archive|warden-store-package-verify|infoZipEnvironment|cwd: temporaryDirectory|release-cli-arguments|normalizeReleaseCliArguments|release-input-file|readBoundedRegularFile|O_NOFOLLOW|MAX_UPLOAD_ARCHIVE_BYTES|MAX_UPLOAD_ARTIFACT_MANIFEST_BYTES|MAX_UPLOAD_EVIDENCE_BYTES|verifyArchiveWithInfoZip|assertTemporaryArchiveUnchanged|TEMPORARY_ARCHIVE_COMPARE_CHUNK_BYTES|temporaryArchiveReadHandle|temporaryArchiveWriteHandle|descriptorPath|warden-release-unzip|INFO_ZIP_ENVIRONMENT|INFO_ZIP_TIMEOUT|killSignal|LC_ALL|read-only seal|chmod\(0o400\)|release-source-tag-message|print-release-source-tag-message|formatReleaseTagMessage|warden\.extension-release-tag\.v1|artifact-manifest-sha256|expected-default-artifact-manifest-sha256|expected-artifact-manifest-sha256|expected-detached-signature-sha256|source tag signed|source tag verifier returned|artifact signature verifier returned|artifact manifest sha256|reviewed artifact manifest differs|detached signature differs|openpgp-signature-policy|reviewed-artifact-signature|verify-reviewed-artifact-signature|release-source-tag|verify-release-source-tag|store-package|verify-store-package|local-dual-extension-release|release-artifact|package-release|verify-release|production-dependency-evidence|bundle-input-evidence|static-input-evidence|release-recipe-input-evidence|expectedArtifactManifestSha256|signedArtifactManifestSha256|artifactReviewSignature|expectedArtifactReviewSignature|artifactReview|reviewedUploadArchive|storePackage|expectedPackageSha256|expectedStorePackageSha256|expectedStoreExtensionId|dualReleaseReport|expectedDualReleaseReportSha256|artifactManifestSha256|boundReleaseFileCount|OFFICIAL_CHROME_WEB_STORE_PUBLISHER_KEY_SHA256|warden\.extension-local-dual-release-rehearsal\.v1|warden\.extension-artifact\.v5|warden\.extension-release-recipe-input-evidence\.v1|OpenPGP verification|OPENPGP_RELEASE_SIGNATURE_POLICY|GIT_GPG_LAUNCHER|signatureCreationDate|signatureExpirationTimestamp' apps/extension/dist; then exit 1; fi && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-extension-dual-release-*' -o -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' -o -name 'warden-release-tag-message-cli-test-*' -o -name 'warden-release-input-file-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-openpgp-signature-policy-test-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-reviewed-artifact-signature-*' -o -name 'warden-reviewed-artifact-signature-test-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed action pins **2/2**, core **700/700**, extension **599/599**, UI
> tokens **11/11**, transaction-budget **8/8**, WebAuthn **1/1**, real Chromium
> **15/15**, Rust **681 passed / 0 failed / 1 ignored**, builds/typechecks, the
> measured Argon2 benchmark, canonical ZIP/five-sidecar verification with **25**
> recipe inputs, independent Info-ZIP parsing, the sequential two-clean-
> checkout rehearsal, emitted release-tooling exclusion, selected temporary-
> directory cleanup checks, diff checks, and both clean-tree guards. Argon2
> elapsed p50/p95 were **920.8/1,001.3 ms**, host-task delay p50/p95 were
> **61.6/64.3 ms**, and password-buffer wiping was true. The final rehearsal
> compared **14** files and produced a **3,810-byte** canonical report with
> SHA-256
> `4e165f3f581d28146c7ab233e90fda44370386034e85b3eb006c93cb43455ca1`.
> At this full-gated SHA the artifact, bundle, recipe, dependency, and static
> sidecar SHA-256 values were respectively
> `560e3a9a362ed1c6d603a8575e7e2ef0640fbad72e04fd36a660d29f704fe078`,
> `4a8e4d70cdad49b375c07c5b7cf82af79cf514e1bb43dfd647519cd97677deac`,
> `086b6ae363adf89420da171327e683fb88c073b7f49516d292102de4b8c5c4ac`,
> `32bd8db7c4cb68ba69e5a88a6bf26ea693bfbe0667d0d6d24bdbf81277b9e7ba`,
> and `0aae28a4f5b3e915b0ae2dbfc1311189e3e90a883d9b502e915e11fdc56112af`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The store verifier
> remained **9,906 bytes** at SHA-256
> `46075410f59d44645b887fb4afd4556ed13eda76a60f4cda3171715b1e980945`.
...
> Anchor test-middleman key mismatch, legacy macro-`cfg`, target-`cfg`, and Rust
> unused-code warnings remained non-fatal. Independent second-model review
> remains **UNVERIFIED**. The rehearsal remains same-host/sequential with a
> shared read-only pnpm store; independent builders are not asserted. No
> production bytes, key, signature, tag, reviewer, Web Store response, upload,
> publish, or deployment were used.
...
>
> Implementation/evidence commit
> `8c4d020d7a4aead78aec9e88d077d6d892c6f6af` passes
> `cwd: temporaryDirectory` to the existing store Info-ZIP call and updates the
> README/release-integrity boundary. From that clean SHA, this exact single-
> contract command exited **0** and printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && node --check apps/extension/scripts/verify-store-package.mjs && node --check apps/extension/test/verify-store-package-infozip.test.mjs && pnpm --filter @warden/extension exec vitest run test/verify-store-package-infozip.test.mjs && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-verify-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed **2/2** tests, syntax, selected cleanup, diff checks, and both clean-
> tree guards. The success probe observed its exact private prefixed `cwd` at
> mode **0700** together with every C72 descriptor/digest/mode/replacement/output
> predicate; the mutation probe still failed closed with empty cleanup.
>
> From that same clean SHA, this exact focused/release command exited **0** and
> printed the same SHA before and after:
>
...
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && pnpm --filter @warden/extension test && pnpm --filter @warden/extension typecheck && pnpm --filter @warden/extension build && if rg -n 'verifyEmbeddedArchiveWithInfoZip|temporary embedded archive|warden-store-package-verify|cwd: temporaryDirectory|release-cli-arguments|normalizeReleaseCliArguments|release-input-file|readBoundedRegularFile|O_NOFOLLOW|verifyArchiveWithInfoZip|assertTemporaryArchiveUnchanged|INFO_ZIP_ENVIRONMENT|INFO_ZIP_TIMEOUT|killSignal|LC_ALL|read-only seal|chmod\(0o400\)|verify-store-package|store-package|verify-release|artifactReview|reviewedUploadArchive|expectedStorePackageSha256|dualReleaseReport|OpenPGP verification|GIT_GPG_LAUNCHER' apps/extension/dist; then exit 1; fi && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' -o -name 'warden-release-tag-message-cli-test-*' -o -name 'warden-release-input-file-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-openpgp-signature-policy-test-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-reviewed-artifact-signature-*' -o -name 'warden-reviewed-artifact-signature-test-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed extension **599/599**, typecheck/build, emitted release-tooling
> exclusion (including the store `cwd` policy), selected cleanup, diff checks,
> and both clean-tree guards. At this SHA the artifact, bundle, recipe,
> dependency, and static sidecar SHA-256 values were respectively
> `11ae73f1fdeb76368ce47bd44127634fe5ef131682227af1d6154636298710a1`,
> `54bb304b81ca341fa5ee82cfe1c6b39720f3c79f3041f1c686c555f4a749cb85`,
> `4719ada6731ce9877e3db1a4d85d70e733281904bd7d3982909e2d262bf89a60`,
> `a1baf26ef0b83510825919b98b4828334b7974ea4c506f02c4846573da19d82c`,
> and `855289ca4791333089a111d237ff6302d6361d0d89a455a002ec38671bf617d2`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The store verifier
> was **9,745 bytes** at SHA-256
> `a809a852b61d4bda2520d532ff53b959be933f9d7e4a7e5504d81ac775e2b5a7`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
> Exact store grammar/signature/digest/output behavior, C72 stable-byte
> handoff, upload-verifier C63–C71 behavior, and provider fixed unavailable are
> unchanged.
>
> Evidence-ledger/full-gated commit
> `19cfdccdb7657a5e5a94abab1c4554f92a2fbca8` ran this exact FULL command from a
> clean tree. It exited **0** and printed that same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && env npm_config_cache=/tmp/warden-npm-cache bash .claude/test-gate.sh && env npm_config_cache=/tmp/warden-npm-cache pnpm --filter @warden/extension release:gate && env npm_config_cache=/tmp/warden-npm-cache pnpm --filter @warden/extension release:dual-local && if rg -n 'verifyEmbeddedArchiveWithInfoZip|temporary embedded archive|warden-store-package-verify|cwd: temporaryDirectory|release-cli-arguments|normalizeReleaseCliArguments|release-input-file|readBoundedRegularFile|O_NOFOLLOW|MAX_UPLOAD_ARCHIVE_BYTES|MAX_UPLOAD_ARTIFACT_MANIFEST_BYTES|MAX_UPLOAD_EVIDENCE_BYTES|verifyArchiveWithInfoZip|assertTemporaryArchiveUnchanged|TEMPORARY_ARCHIVE_COMPARE_CHUNK_BYTES|temporaryArchiveReadHandle|temporaryArchiveWriteHandle|descriptorPath|warden-release-unzip|INFO_ZIP_ENVIRONMENT|INFO_ZIP_TIMEOUT|killSignal|LC_ALL|read-only seal|chmod\(0o400\)|release-source-tag-message|print-release-source-tag-message|formatReleaseTagMessage|warden\.extension-release-tag\.v1|artifact-manifest-sha256|expected-default-artifact-manifest-sha256|expected-artifact-manifest-sha256|expected-detached-signature-sha256|source tag signed|source tag verifier returned|artifact signature verifier returned|artifact manifest sha256|reviewed artifact manifest differs|detached signature differs|openpgp-signature-policy|reviewed-artifact-signature|verify-reviewed-artifact-signature|release-source-tag|verify-release-source-tag|store-package|verify-store-package|local-dual-extension-release|release-artifact|package-release|verify-release|production-dependency-evidence|bundle-input-evidence|static-input-evidence|release-recipe-input-evidence|expectedArtifactManifestSha256|signedArtifactManifestSha256|artifactReviewSignature|expectedArtifactReviewSignature|artifactReview|reviewedUploadArchive|storePackage|expectedPackageSha256|expectedStorePackageSha256|expectedStoreExtensionId|dualReleaseReport|expectedDualReleaseReportSha256|artifactManifestSha256|boundReleaseFileCount|OFFICIAL_CHROME_WEB_STORE_PUBLISHER_KEY_SHA256|warden\.extension-local-dual-release-rehearsal\.v1|warden\.extension-artifact\.v5|warden\.extension-release-recipe-input-evidence\.v1|OpenPGP verification|OPENPGP_RELEASE_SIGNATURE_POLICY|GIT_GPG_LAUNCHER|signatureCreationDate|signatureExpirationTimestamp' apps/extension/dist; then exit 1; fi && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-extension-dual-release-*' -o -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' -o -name 'warden-release-tag-message-cli-test-*' -o -name 'warden-release-input-file-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-openpgp-signature-policy-test-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-reviewed-artifact-signature-*' -o -name 'warden-reviewed-artifact-signature-test-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
...
> checkout rehearsal, emitted release-tooling exclusion, selected temporary-
> directory cleanup checks, diff checks, and both clean-tree guards. Argon2
> elapsed p50/p95 were **925.7/1,020.3 ms**, host-task delay p50/p95 were
> **61.8/63.4 ms**, and password-buffer wiping was true. The final rehearsal
> compared **14** files and produced a **3,810-byte** canonical report with
> SHA-256
> `013e31be9ce3ad61dba002e0f13ad39c899d1656caccb9d46440a50bbe65c8d5`.
> At this full-gated SHA the artifact, bundle, recipe, dependency, and static
> sidecar SHA-256 values were respectively
> `1a505dd26e03b606dac2e45970960aa9bf2459283167d1b2ae6c1009c53e5075`,
> `52344827859dac1598564d88d3263485af4301d50e562bec2f5979f71146851e`,
> `3a54957d89728d09369ad9223ce25521e6508856b3f8e24f43b744e4051ec7de`,
> `bd505065f3c12b467b0420b0a753aff6077817906b57593ff349c0aa405f17ac`,
> and `367cd738a8489088cbaddc7452f7bbc2652c8236ddc16d2fdf370b1a4605aa58`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The store verifier
> remained **9,745 bytes** at SHA-256
> `a809a852b61d4bda2520d532ff53b959be933f9d7e4a7e5504d81ac775e2b5a7`.
> ZIP SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and payload-tree SHA-256 remained
...
>
> Implementation/evidence commit
> `a99054d5c5b51a30d564a3d2b6081769ba8d5a2e` uses exclusive
> `O_CREAT|O_EXCL|O_RDWR|O_NOFOLLOW` construction at **0600**, syncs and
> identity-checks a same-inode `O_RDONLY|O_NOFOLLOW` handle, closes the writer,
> seals/verifies **0400**, unlinks the name, passes the live descriptor, and
> positionally compares every original byte after parser success. A direct
> module entry guard keeps the CLI executable while exposing only the embedded-
> archive handoff for its mutation probe. README and release-integrity prose
> state the store-specific residual environment/`cwd`/runtime boundary.
>
> From that clean implementation/evidence SHA, this exact single-contract
...
>
> It passed **2/2** tests, syntax, selected cleanup, diff checks, and both clean-
> tree guards. The first probe saw the independently expected embedded digest
> through `/proc/<pid>/fd/<fd>`, access mode zero, inode mode **0400**, a refused
> rename, successful CLI completion, and empty cleanup. The second restored
> mode **0600** through procfs, rewrote the exact-length inode, exited zero, and
> was rejected by the post-parser byte comparison with empty cleanup.
>
> From that same clean SHA, this exact focused/release command exited **0** and
> printed the same SHA before and after:
>
...
> payload files, **60** production/peer components, **4** JavaScript bundles,
> **101** positive bundle inputs, **4** static inputs, **25** exact recipe inputs,
> the canonical unpacked tree, real upload Info-ZIP parsing, selected cleanup,
...
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && pnpm --filter @warden/extension test && pnpm --filter @warden/extension typecheck && pnpm --filter @warden/extension build && if rg -n 'verifyEmbeddedArchiveWithInfoZip|temporary embedded archive|warden-store-package-verify|release-cli-arguments|normalizeReleaseCliArguments|release-input-file|readBoundedRegularFile|O_NOFOLLOW|MAX_UPLOAD_ARCHIVE_BYTES|MAX_UPLOAD_ARTIFACT_MANIFEST_BYTES|MAX_UPLOAD_EVIDENCE_BYTES|verifyArchiveWithInfoZip|assertTemporaryArchiveUnchanged|TEMPORARY_ARCHIVE_COMPARE_CHUNK_BYTES|temporaryArchiveReadHandle|temporaryArchiveWriteHandle|descriptorPath|warden-release-unzip|cwd: temporaryDirectory|INFO_ZIP_ENVIRONMENT|INFO_ZIP_TIMEOUT|killSignal|LC_ALL|read-only seal|chmod\(0o400\)|release-source-tag-message|print-release-source-tag-message|formatReleaseTagMessage|warden\.extension-release-tag\.v1|artifact-manifest-sha256|expected-default-artifact-manifest-sha256|expected-artifact-manifest-sha256|expected-detached-signature-sha256|source tag signed|source tag verifier returned|artifact signature verifier returned|reviewed artifact manifest differs|detached signature differs|verify-release-source-tag|release-source-tag|verify-reviewed-artifact-signature|reviewed-artifact-signature|verify-store-package|store-package|verify-release|expectedArtifactManifestSha256|signedArtifactManifestSha256|artifactManifestSha256|artifactReview|reviewedUploadArchive|storePackage|expectedStorePackageSha256|dualReleaseReport|OpenPGP verification|GIT_GPG_LAUNCHER' apps/extension/dist; then exit 1; fi && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' -o -name 'warden-release-tag-message-cli-test-*' -o -name 'warden-release-input-file-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-openpgp-signature-policy-test-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-reviewed-artifact-signature-*' -o -name 'warden-reviewed-artifact-signature-test-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed extension **599/599**, typecheck/build, emitted release-tooling
> exclusion (including the store descriptor helper), selected cleanup, diff
> checks, and both clean-tree guards. At this SHA the artifact, bundle, recipe,
> dependency, and static sidecar SHA-256 values were respectively
> `d164fa484bf640b5f0efd2052f98e86476929c5e990b4df95a028c2b2f44ce36`,
> `f2f2dadfd2e4d7a9da0a6d11bbc7f12e3a66ed353d18c32589acc2a15ab19cd5`,
> `c5e5863e03b10b35f88f97fb2d8ac3c2d287a654fcde1bef066ded879b918267`,
> `7784e81ab1829c260013d7a56fd9e557faf8e2116807081fc57d9452f2631479`,
> and `0bd01101e4f6f828e01c58bad16f8a5836e6615ef7210b932151c1ac7dd813b7`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The store verifier
> was **9,714 bytes** at SHA-256
> `03c8eced2f4d8dbb95b2dac389dec66136373f69ff7a0289dfe67ebb60fe1624`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
> Exact standalone/composed store grammar, digest/id/signature checks, output,
> upload-verifier C63–C71 behavior, and provider fixed unavailable are unchanged.
>
> After committing that evidence ledger, the exact FULL command below ran from
> clean SHA `bd717bdf19ef9986522e1180cd2c67b8dba96660`, exited **0**, and
> printed that same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && env npm_config_cache=/tmp/warden-npm-cache bash .claude/test-gate.sh && env npm_config_cache=/tmp/warden-npm-cache pnpm --filter @warden/extension release:gate && env npm_config_cache=/tmp/warden-npm-cache pnpm --filter @warden/extension release:dual-local && if rg -n 'verifyEmbeddedArchiveWithInfoZip|temporary embedded archive|warden-store-package-verify|release-cli-arguments|normalizeReleaseCliArguments|release-input-file|readBoundedRegularFile|O_NOFOLLOW|MAX_UPLOAD_ARCHIVE_BYTES|MAX_UPLOAD_ARTIFACT_MANIFEST_BYTES|MAX_UPLOAD_EVIDENCE_BYTES|verifyArchiveWithInfoZip|assertTemporaryArchiveUnchanged|TEMPORARY_ARCHIVE_COMPARE_CHUNK_BYTES|temporaryArchiveReadHandle|temporaryArchiveWriteHandle|descriptorPath|warden-release-unzip|cwd: temporaryDirectory|INFO_ZIP_ENVIRONMENT|INFO_ZIP_TIMEOUT|killSignal|LC_ALL|read-only seal|chmod\(0o400\)|release-source-tag-message|print-release-source-tag-message|formatReleaseTagMessage|warden\.extension-release-tag\.v1|artifact-manifest-sha256|expected-default-artifact-manifest-sha256|expected-artifact-manifest-sha256|expected-detached-signature-sha256|source tag signed|source tag verifier returned|artifact signature verifier returned|artifact manifest sha256|reviewed artifact manifest differs|detached signature differs|openpgp-signature-policy|reviewed-artifact-signature|verify-reviewed-artifact-signature|release-source-tag|verify-release-source-tag|store-package|verify-store-package|local-dual-extension-release|release-artifact|package-release|verify-release|production-dependency-evidence|bundle-input-evidence|static-input-evidence|release-recipe-input-evidence|expectedArtifactManifestSha256|signedArtifactManifestSha256|artifactReviewSignature|expectedArtifactReviewSignature|artifactReview|reviewedUploadArchive|storePackage|expectedPackageSha256|expectedStorePackageSha256|expectedStoreExtensionId|dualReleaseReport|expectedDualReleaseReportSha256|artifactManifestSha256|boundReleaseFileCount|OFFICIAL_CHROME_WEB_STORE_PUBLISHER_KEY_SHA256|warden\.extension-local-dual-release-rehearsal\.v1|warden\.extension-artifact\.v5|warden\.extension-release-recipe-input-evidence\.v1|OpenPGP verification|OPENPGP_RELEASE_SIGNATURE_POLICY|GIT_GPG_LAUNCHER|signatureCreationDate|signatureExpirationTimestamp' apps/extension/dist; then exit 1; fi && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-extension-dual-release-*' -o -name 'warden-store-package-infozip-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' -o -name 'warden-release-tag-message-cli-test-*' -o -name 'warden-release-input-file-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-openpgp-signature-policy-test-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-reviewed-artifact-signature-*' -o -name 'warden-reviewed-artifact-signature-test-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
...
> **15/15**, Rust **681 passed / 0 failed / 1 ignored**, builds/typechecks, the
> measured Argon2 benchmark, canonical ZIP/five-sidecar verification with
> **25** recipe inputs, the upload verifier's bounded/minimal-environment
> Info-ZIP parsing, the real sequential two-clean-checkout rehearsal, emitted
> release-tooling exclusion, selected temp-directory cleanup, diff checks, and
> both clean-tree guards. The Argon2 elapsed p50/p95 were **930.0/945.9 ms**,
> host-task delay p50/p95 were **64.4/67.5 ms**, and password-buffer wiping was
> true. The final rehearsal compared **14** files and produced a **3,810-byte**
> canonical report with SHA-256
> `1da97905257328173f6e34ba219e9d520d41c202f9efd0df0aef99c584b8c544`.
> At the full-gated SHA the artifact, bundle, recipe, dependency, and static
> sidecar SHA-256 values were respectively
> `7b8fd5a6e78490d0168723fccea945d5998df512cf693b9bd890c06f8f31129e`,
> `18e481b2f0ae0ced83cb5f4fd651762b4d704864d81491739d69319da72396b1`,
> `f26f2819e46de76b643d101f4366d6a3b72f34467e569a39eab2c8a757c370f2`,
> `88a12e499e25670b90b8cce064fb1d1bd04ccaa9c01eccc686c2612bf904eebd`,
> and `e99544a58f1ea4f236cd6907b034226875ee7cf2aa6b3b431a7694109f1e46c6`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The store verifier
> remained **9,714 bytes** at SHA-256
> `03c8eced2f4d8dbb95b2dac389dec66136373f69ff7a0289dfe67ebb60fe1624`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
...
>
> Implementation/evidence commit
> `a539e21e66c35b392f921d6c2c8f08d7d6108b28` calculates the contracted
> timeout from the already stable-read archive length and passes it with
> `killSignal: "SIGKILL"` to the existing Info-ZIP `execFile` call. README and
> release-integrity prose state the direct-child/non-process-group boundary.
>
> From that clean implementation/evidence SHA, this exact single-contract
> command exited **0** and printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && node --check apps/extension/scripts/verify-release.mjs && node --check apps/extension/test/verify-release-cli.test.mjs && pnpm --filter @warden/extension exec vitest run test/verify-release-cli.test.mjs -t "kills a stalled independent unzip direct child within its archive deadline" && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed one test, skipped twelve, syntax, selected cleanup, diff checks, and
> both clean-tree guards in **7.26 s**. The fake direct child wrote its start
> marker, was killed/fail-closed in about **6.9 s**, did not write its completion
> marker, and left no private directory.
>
> From that same clean SHA, this exact verifier-file command exited **0** and
> printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && pnpm --filter @warden/extension exec vitest run test/verify-release-cli.test.mjs && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed **13/13** tests in **14.54 s**, including every C71/C70/C69/C68/
> C67/C66/C65/C64/C63 timeout, environment, directory, descriptor, inode,
> mutation, name-replacement, stable-copy, failure-cleanup, grammar, and input-
> bound probe.
>
> From that same clean SHA, this exact focused/release command exited **0** and
> printed the same SHA before and after:
...
> ```
>
> It passed **97/97** focused tests, typecheck, canonical verification of **8**
> payload files, **60** production/peer components, **4** JavaScript bundles,
> **101** positive bundle inputs, **4** static inputs, **25** exact recipe inputs,
> the canonical unpacked tree, real bounded/minimal-environment Info-ZIP parsing
> from the private directory, selected cleanup, diff checks, and both clean-tree
> guards.
>
> From that same clean SHA, this exact extension-wide command exited **0** and
> printed the same SHA before and after:
...
> ```
>
> It passed extension **597/597**, typecheck/build, emitted release-tooling
> exclusion (including timeout/kill policy), selected cleanup, diff checks, and
> both clean-tree guards. At this SHA the artifact, bundle, recipe, dependency,
> and static sidecar SHA-256 values were respectively
> `cdc813c9e0f34c9a99c6d6471fa2cf85c546d7b4acf71bf20ba56f2b3b342b28`,
> `d43a855bfd3782cbd32e2a55d8afacfed927f6caab04d90b1a9d252397c30cf1`,
> `75a6f52ab8ef71457020fbaa7bec2711c037ba236cf30f20264aaf4d67a06848`,
> `c281cf6f1957cbe05f45b71e57ef6e3111e34e0ca61aea772f1192e312316e0a`,
> and `bf3b59a48e5a1678cbec13b0181718408b11a3bf14155ade484482a54b3ec96d`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The verifier was
> **11,877 bytes** at SHA-256
> `7215e3959a9cf719ce2bad3d5919f49961eaf214bfeb0b8c26f7d723f495c42a`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
> Exact direct/pnpm **0/6/7** grammar, one-separator behavior, default paths,
> input/verification order, optional unpacked tree, canonical checks, successful
> output, construction/sealed/directory/environment modes, and recipe membership
> are otherwise unchanged. The verifier was already recipe-bound.
>
> After committing that evidence ledger, the exact FULL command below ran from
> clean SHA `7facbd975fb46649c3950c9ae47fa3fe7ddd573f`, exited **0**, and
> printed that same SHA before and after:
>
> ```sh
...
> ```
>
> It passed action pins **2/2**, core **700/700**, extension **597/597**, UI
> tokens **11/11**, transaction-budget **8/8**, WebAuthn **1/1**, real Chromium
> **15/15**, Rust **681 passed / 0 failed / 1 ignored**, builds/typechecks, the
> measured Argon2 benchmark, canonical ZIP/five-sidecar verification with
> **25** recipe inputs, bounded/minimal-environment Info-ZIP parsing, the real
> sequential two-clean-checkout rehearsal, emitted release-tooling exclusion,
> selected temp-directory cleanup, diff checks, and both clean-tree guards. The
> Argon2 elapsed p50/p95 were **920.4/961.2 ms**, host-task delay p50/p95 were
> **62.2/68.7 ms**, and password-buffer wiping was true. The final rehearsal
> compared **14** files and produced a **3,810-byte** canonical report with
> SHA-256
> `19819cdb2df387c9d861ed165a71e4a4541448ab58afdb603634829032baa7a5`.
> At the full-gated SHA the artifact, bundle, recipe, dependency, and static
> sidecar SHA-256 values were respectively
> `6a34ccb0d65f75ed13f0d0f188b05ec2d261022f74c20a58881ede8cc494e037`,
> `56dd9f62768cd52c5e644450ba4686790f6329e21ab3ccbf718b9d88db678f64`,
> `efc2b82f0ca1481e8ae5c8109464c12240ed8229213215747a9ee241f0444810`,
> `abdfd57c8649f7823c755f4823086e09665880b213ed2fb4002801bca62dbe84`,
> and `a90b7a276e247ef8cf945010aaf0b2aefe07632eb09fd3ba9877f172a5752a73`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The verifier
> remained **11,877 bytes** at SHA-256
> `7215e3959a9cf719ce2bad3d5919f49961eaf214bfeb0b8c26f7d723f495c42a`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
...
>
> Implementation/evidence commit
> `c78649e7c63211697fb082b7a563f65e124b9e93` passes an explicit frozen object
> containing only inherited `PATH` (with the contracted absent-value fallback),
> `LANG=C`, and `LC_ALL=C` to Info-ZIP. The four older generated executable
> probes now embed their ephemeral fixture paths and use a fixture file for the
> nonzero-exit branch, so no production test-only environment exception exists.
> README and release-integrity prose state the cooperative, non-sandbox boundary.
>
> From that clean implementation/evidence SHA, this exact single-contract
> command exited **0** and printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && node --check apps/extension/scripts/verify-release.mjs && node --check apps/extension/test/verify-release-cli.test.mjs && pnpm --filter @warden/extension exec vitest run test/verify-release-cli.test.mjs -t "gives independent unzip only the contracted child environment" && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed one test, skipped eleven, syntax, selected cleanup, diff checks, and
> both clean-tree guards in **1.40 s**. The fake parser measured exactly the
> three contracted environment keys and values, absence of the unrelated marker
> and `TMPDIR`, exact stable bytes, descriptor access mode zero, inode mode
> **0400**, and the exact private **0700** working directory.
>
> From that same clean SHA, this exact verifier-file command exited **0** and
> printed the same SHA before and after:
>
...
> ```
>
> It passed **12/12** tests in **7.43 s**, including every C70/C69/C68/C67/C66/
> C65/C64/C63 environment, directory, descriptor, inode, mutation, name-
> replacement, stable-copy, failure-cleanup, grammar, and input-bound probe.
>
> From that same clean SHA, this exact focused/release command exited **0** and
> printed the same SHA before and after:
>
...
> ```
>
> It passed **96/96** focused tests, typecheck, canonical verification of **8**
> payload files, **60** production/peer components, **4** JavaScript bundles,
> **101** positive bundle inputs, **4** static inputs, **25** exact recipe inputs,
> the canonical unpacked tree, real minimal-environment Info-ZIP parsing from the
> private directory, selected cleanup, diff checks, and both clean-tree guards.
>
> From that same clean SHA, this exact extension-wide command exited **0** and
> printed the same SHA before and after:
>
...
> ```
>
> It passed extension **596/596**, typecheck/build, emitted release-tooling
> exclusion (including the new environment policy), selected cleanup, diff
> checks, and both clean-tree guards. At this SHA the artifact, bundle, recipe,
> dependency, and static sidecar SHA-256 values were respectively
> `39238a871cf97ebb9b94b13f768345300eab4d14ca55a43b8ffd3f2fe2e924c4`,
> `219244002356f50a99179cae2562cb1e7653ee9df50fae732649d2126abe702c`,
> `63f6956782926892732415fa91f6ff97141692710103ac89c3ed832502c465d3`,
> `cc2209a2d0b73dcc4d530dd7c1ccb23f9c1a72f7a4950bd758aafd5f5cdc7ca2`,
> and `6928cf0eb7cb99b7c4f08435c2849854e2a61f59f2d62e8dd866f2168e2828be`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The verifier was
> **11,437 bytes** at SHA-256
...
>
> Implementation/evidence commit
> `a206228cacd95197bbd677af6309f1a130db2f2a` sets `cwd` to the still-live
> private temporary directory in the Info-ZIP `execFile` options. The archive
> name remains unlinked before invocation, and all C68 handle/seal/compare/
> cleanup behavior is unchanged. README and release-integrity prose state that
> the working directory limits incidental effects but is not a sandbox.
>
> From that clean implementation/evidence SHA, this exact single-contract
> command exited **0** and printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && node --check apps/extension/scripts/verify-release.mjs && node --check apps/extension/test/verify-release-cli.test.mjs && pnpm --filter @warden/extension exec vitest run test/verify-release-cli.test.mjs -t "runs independent unzip on the sealed descriptor from its private directory" && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed one test, skipped ten, syntax, selected cleanup, diff checks, and
> both clean-tree guards in **1.31 s**. The fake parser measured exact stable
> bytes, descriptor access mode **0**, inode mode **0400**, and the exact private
> `warden-release-unzip-*` working directory at mode **0700**.
>
> From that same clean SHA, this exact focused/release command exited **0** and
> printed the same SHA before and after:
>
...
> payload files, **60** production/peer components, **4** JavaScript bundles,
> **101** positive bundle inputs, **4** static inputs, **25** exact recipe inputs,
> the canonical unpacked tree, real Info-ZIP parsing from the private directory,
> selected cleanup, diff checks, and both clean-tree guards. C68/C67/C66/C65/
> C64/C63 descriptor, mode, mutation, name-replacement, and stable-copy probes
> remained green.
>
> From that same clean SHA, this exact extension-wide command exited **0** and
> printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && pnpm --filter @warden/extension test && pnpm --filter @warden/extension typecheck && pnpm --filter @warden/extension build && if rg -n 'release-cli-arguments|normalizeReleaseCliArguments|release-input-file|readBoundedRegularFile|O_NOFOLLOW|MAX_UPLOAD_ARCHIVE_BYTES|MAX_UPLOAD_ARTIFACT_MANIFEST_BYTES|MAX_UPLOAD_EVIDENCE_BYTES|verifyArchiveWithInfoZip|assertTemporaryArchiveUnchanged|TEMPORARY_ARCHIVE_COMPARE_CHUNK_BYTES|temporaryArchiveReadHandle|temporaryArchiveWriteHandle|descriptorPath|warden-release-unzip|cwd: temporaryDirectory|read-only seal|chmod\(0o400\)|release-source-tag-message|print-release-source-tag-message|formatReleaseTagMessage|warden\.extension-release-tag\.v1|artifact-manifest-sha256|expected-default-artifact-manifest-sha256|expected-artifact-manifest-sha256|expected-detached-signature-sha256|source tag signed|source tag verifier returned|artifact signature verifier returned|reviewed artifact manifest differs|detached signature differs|verify-release-source-tag|release-source-tag|verify-reviewed-artifact-signature|reviewed-artifact-signature|verify-store-package|store-package|verify-release|expectedArtifactManifestSha256|signedArtifactManifestSha256|artifactManifestSha256|artifactReview|reviewedUploadArchive|storePackage|expectedStorePackageSha256|dualReleaseReport|OpenPGP verification|GIT_GPG_LAUNCHER' apps/extension/dist; then exit 1; fi && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' -o -name 'warden-release-tag-message-cli-test-*' -o -name 'warden-release-input-file-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-openpgp-signature-policy-test-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-reviewed-artifact-signature-*' -o -name 'warden-reviewed-artifact-signature-test-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed extension **595/595**, typecheck/build, emitted release-tooling
> exclusion, selected cleanup, diff checks, and both clean-tree guards. At this
> SHA the artifact, bundle, recipe, dependency, and static sidecar SHA-256 values
> were respectively
> `2b0396789a0d19325bdc9fca1a6fb0203ffc256b77ecec0f6be69609c81b6901`,
> `fc8e99f657d85264a93a80892e2554fc4d5a8ea200a1f9e30f92162410474dcf`,
> `73fdf3d200b1c4ed8e1bb70569d1069dcc2ad3e9da7b3268aefe710031f76d1a`,
> `97d7d33918e81f603761e4520cc7f57c303ade83d78870ae45166dd4061a9b72`,
> and `730efbf65e2bced3a2ffe957e54364e08986d55c88b600437a519239f447c75f`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The verifier was
> **11,282 bytes** at SHA-256
> `77a028e1eb8a3f196386267e69243c84eed9336e97bca7a27f48149a0c86fd93`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
> Exact direct/pnpm **0/6/7** grammar, one-separator behavior, default paths,
...
>
> Evidence-ledger/full-gated commit
> `c500cb88815d5a20e18f06b37918b994a9d57799` records the command before its
> execution. From that clean SHA, this exact FULL command exited **0** and
> printed the same SHA before and after:
>
> ```sh
...
> ```
>
> It passed action pins **2/2**, core **700/700**, extension **595/595**, UI
> tokens **11/11**, transaction-budget **8/8**, WebAuthn **1/1**, real Chromium
> **15/15**, and Rust **681 passed / 0 failed / 1 ignored**, plus builds,
> typechecks, the canonical release gate, real Info-ZIP parsing from the private
> directory, the sequential two-clean-checkout rehearsal, emitted release-
> tooling exclusion, selected cleanup, diff checks, and both clean-tree guards.
> The measured Argon2 elapsed p50/p95 were **907.0/1017.3 ms**, host-task delay
> p50/p95 were **61.4/67.9 ms**, and password-buffer wiping was true. Known
> Anchor test-middleman key mismatch, legacy macro-`cfg`, target-`cfg`, and Rust
> unused-code warnings remained non-fatal.
>
> The final rehearsal compared **14** files and produced a **3,810-byte**
> canonical report with SHA-256
> `a2718cdedad9f771d2fb355ee2d9684515d7e7d5420a03f4cccd5f522f225e95`.
> At the full-gated SHA the artifact, bundle, recipe, dependency, and static
> sidecar SHA-256 values were respectively
> `59fe5ed799c1045d5e98bbed9fdf6f2d91ada53d04b5f16f974128ae956908d1`,
> `4881305e398c226b42f606b081b181849f031b8a428d63a3fd585aea658d50c5`,
> `7b53eb13765b784770a8d3e0696c0ec6376d3d03eaff7088c3e6184b992cdd29`,
> `9c50a53a66744c5044f46381df5c5c1c558fa54107b98d0dde075e2a9d25e5eb`,
> and `7802d0230d5149f8e9aaad5c2df632a62a587aec2c912236715c3ad871452181`.
> The recipe sidecar remained **5,126 bytes** with **25** inputs; the verifier
> remained **11,282 bytes** at SHA-256
> `77a028e1eb8a3f196386267e69243c84eed9336e97bca7a27f48149a0c86fd93`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
...
> external. No invariant or production-trust status moves. Independent second-
> model review remains **UNVERIFIED**.
...
>
> Implementation commit `c57e14cdd9ea15556e2e5fe3ae0f509bc360165e`
> closes the exclusive construction writer, applies `fchmod(0400)` through the
> live read handle, and performs a new bigint `fstat` requiring unchanged regular-
> file type, device, inode, and size plus exact mode **0400** before unlink/exec.
> README and release-integrity prose state the cooperative-host boundary.
>
> The first broader focused run at that SHA was not green: it passed **93/95**
> tests and exposed two older expectations that intentionally intersect C68. The
> C64 probe still expected parser-time mode **0600**, and the C66 hostile rewrite
> was denied before it could exercise the post-parser comparator. Corrected
> implementation/evidence commit
> `a223edf8a1d8d5c11381756e9fd013ff4b8f5026` changes the first expectation to
> **0400** and makes the hostile fake parser explicitly restore owner-write mode
> before rewriting. This preserves the distinct C66 check and proves that C68
> does not constrain a malicious file owner.
>
> From clean corrected implementation/evidence SHA
> `a223edf8a1d8d5c11381756e9fd013ff4b8f5026`, this exact single-contract
> command exited **0** and printed the same SHA before and after:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && node --check apps/extension/scripts/verify-release.mjs && node --check apps/extension/test/verify-release-cli.test.mjs && pnpm --filter @warden/extension exec vitest run test/verify-release-cli.test.mjs -t "gives independent unzip a read-only descriptor on a sealed inode" && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
> ```
>
> It passed one test, skipped ten, syntax, selected cleanup, diff checks, and
> both clean-tree guards in **1.32 s**. The fake parser measured exact stable
> bytes, descriptor access mode **0**, and underlying inode mode **0400**.
>
> From that same clean SHA, this exact focused/release command exited **0** and
> printed the same SHA before and after:
>
...
> inputs, the canonical unpacked tree, real Info-ZIP parsing through the sealed
> read descriptor, selected cleanup, diff checks, and both clean-tree guards.
> The C66 fake restored **0600**, rewrote every byte at the same length, exited
> zero, and remained rejected by the exact post-parser comparison. C65/C64/C63
> stable descriptor/copy probes also remained green.
>
> From that same clean SHA, this exact extension-wide command exited **0** and
> printed the same SHA before and after:
>
...
> SHA the artifact, bundle, recipe, dependency, and static sidecar SHA-256 values
> were respectively
> `90ab96c65c2f5e2fe4bf63bafcc1425b2b2eb671fe1dab5fe423203854f65421`,
> `d3108ad46620d354bdb10b94a50b68dbd0d53fba13b4e0c023472c7ad283ba0e`,
> `3c568de28c0f4093c603c2204cd9e4dc935e907353b94ae2b30657a3369d3335`,
> `85e639016a0d477617854992726d79280c0f60ef498b992acb8ffe6d1efce521`,
> and `0de08e45504dcc6f59d30f2afce0f4c91edbb178e37d125b844ac728ac9bf77c`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs. The verifier was
> **11,251 bytes** at SHA-256
> `b964b6dbe2d39920f4ca849b479d3c2e5ac034f26c9fa0fde1b4e0fc64e9b5cc`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
> Exact direct/pnpm **0/6/7** grammar, one-separator behavior, default paths,
> input and verification order, optional unpacked tree, canonical checks,
> successful output, initial **0600** construction and **0700** directory modes,
...
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
>
> Evidence-ledger commit `47d419dae0dda40ad6ca461ab7cd81dc3ba32308`
> was clean before this exact FULL command:
>
> ```sh
> git rev-parse HEAD && test -z "$(git status --porcelain)" && env npm_config_cache=/tmp/warden-npm-cache bash .claude/test-gate.sh && env npm_config_cache=/tmp/warden-npm-cache pnpm --filter @warden/extension release:gate && env npm_config_cache=/tmp/warden-npm-cache pnpm --filter @warden/extension release:dual-local && if rg -n 'release-cli-arguments|normalizeReleaseCliArguments|release-input-file|readBoundedRegularFile|O_NOFOLLOW|MAX_UPLOAD_ARCHIVE_BYTES|MAX_UPLOAD_ARTIFACT_MANIFEST_BYTES|MAX_UPLOAD_EVIDENCE_BYTES|verifyArchiveWithInfoZip|assertTemporaryArchiveUnchanged|TEMPORARY_ARCHIVE_COMPARE_CHUNK_BYTES|temporaryArchiveReadHandle|temporaryArchiveWriteHandle|descriptorPath|warden-release-unzip|release-source-tag-message|print-release-source-tag-message|formatReleaseTagMessage|warden\.extension-release-tag\.v1|artifact-manifest-sha256|expected-default-artifact-manifest-sha256|expected-artifact-manifest-sha256|expected-detached-signature-sha256|source tag signed|source tag verifier returned|artifact signature verifier returned|artifact manifest sha256|reviewed artifact manifest differs|detached signature differs|openpgp-signature-policy|reviewed-artifact-signature|verify-reviewed-artifact-signature|release-source-tag|verify-release-source-tag|store-package|verify-store-package|local-dual-extension-release|release-artifact|package-release|verify-release|production-dependency-evidence|bundle-input-evidence|static-input-evidence|release-recipe-input-evidence|expectedArtifactManifestSha256|signedArtifactManifestSha256|artifactReviewSignature|expectedArtifactReviewSignature|artifactReview|reviewedUploadArchive|storePackage|expectedPackageSha256|expectedStorePackageSha256|expectedStoreExtensionId|dualReleaseReport|expectedDualReleaseReportSha256|artifactManifestSha256|boundReleaseFileCount|OFFICIAL_CHROME_WEB_STORE_PUBLISHER_KEY_SHA256|warden\.extension-local-dual-release-rehearsal\.v1|warden\.extension-artifact\.v5|warden\.extension-release-recipe-input-evidence\.v1|OpenPGP verification|OPENPGP_RELEASE_SIGNATURE_POLICY|GIT_GPG_LAUNCHER|signatureCreationDate|signatureExpirationTimestamp' apps/extension/dist; then exit 1; fi && test -z "$(find /tmp -maxdepth 1 -type d \( -name 'warden-extension-dual-release-*' -o -name 'warden-release-verify-cli-test-*' -o -name 'warden-release-unzip-*' -o -name 'warden-release-tag-message-cli-test-*' -o -name 'warden-release-input-file-test-*' -o -name 'warden-release-source-cli-test-*' -o -name 'warden-store-package-cli-test-*' -o -name 'warden-store-package-verify-*' -o -name 'warden-release-source-gpg-launcher-*' -o -name 'warden-openpgp-signature-policy-test-*' -o -name 'warden-release-source-tag-test-*' -o -name 'warden-reviewed-artifact-signature-*' -o -name 'warden-reviewed-artifact-signature-test-*' \) -print -quit)" && git diff --check && git diff --exit-code && test -z "$(git status --porcelain)" && git rev-parse HEAD
...
> **681 passed / 0 failed / 1 ignored**, builds and typechecks, measured Argon2,
> canonical release packaging/verification, independent Info-ZIP parsing through
> the read-only descriptor plus exact post-parser comparison, same-host
> sequential dual reproduction, emitted release-tooling exclusion, all selected
> cleanup checks, diff checks, and both clean-tree guards. Argon2 elapsed p50/p95
> were **931.7/994.8 ms**; host-task delay p50/p95 were **63.8/68.9 ms**;
> password-buffer wiping was true.
>
> The final rehearsal compared **14** files and produced a **3,810-byte**
> canonical report with SHA-256
> `41de546a482659c5e93714b077e024e9624bc95a05c7e47cfd931f909d24465d`.
> At the full-gated SHA the artifact, bundle, recipe, dependency, and static
> sidecar SHA-256 values were respectively
> `122602d570c1f96dde77cad2969fd9135fa73d9138f21968051a9cce0be66286`,
> `584c0a335b856aad3695c6268790a2b0523b67767dd56d04f52b20a97a0537e8`,
> `ace66ee9b6a4d0a1e6325f9fa5d45e127b09a60ed4f0572538a16ae7301d5b63`,
> `658600631d032b6fc9289ea9266fd2cc3b14a5d2b19cc4be67118250c3104987`,
> and `1965f91efa7ca27b362d837f304407c75c89f613d3e6934452925fc0689f445a`.
> The recipe sidecar remained **5,126 bytes** with 25 inputs; the changed
> verifier remained **10,826 bytes** at SHA-256
> `d07b719ce4bed80eb2cfe13f037754928636588c8cf5ef1b92cb204c121b73e8`.
> ZIP SHA-256 and payload-tree SHA-256 remained
> `ce1b3a4792cd28def0b336d99a990bda3141c26f0b625b206163d505aca2c844`
> and `f0e7ef2c6f3d1133b5e40557a014a656ccd1fe0cb7590632973b8e33a447a879`.
...
> Independent second-model review remains **UNVERIFIED**. C67 is closed.
