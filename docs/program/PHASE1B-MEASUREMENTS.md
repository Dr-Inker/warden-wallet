...
**DEBT DISCHARGED 2026-08-23 — the owed round ran clean.** `scripts/review.sh
9a427aa35dd6e1f89c3fe00e5b1dd482118a87c6 003968100ad1c9c3b92d197bf7672baf6e216533
--kind task-diff` completed with no content-filter block (same lane, same
profile, ~24 h later — consistent with a transient filter, not a property of the
diff). gpt-5.6-sol @ `max`, 67 seeded invariants, thread
`phase1b/grok-remediation/round-1`, artefact
`.superpowers/reviews/003968100ad1-20260823T090301Z.json`
(sha256 `454afe0e…386a`), recorded by `scripts/append-review-run.mjs` in
REVIEW-RUNS.jsonl + REVIEW-SCORECARD.jsonl. **4 findings, all ACCEPTED on
adjudication** — WRDF-0104/0105 (important), WRDF-0106/0107 (minor); see the
round-1 remediation section below. The GROK remediation is therefore
review-closed as of this round, with those 4 findings carried as its follow-on
work rather than as unreviewed risk.

### Gate evidence (WRDF-0107)

WRDF-0107 correctly flagged that "full gate green" appeared repeatedly with no
command and no SHA behind it, and that a first attempt at this very entry
reported `exit code 0` from a command piped through `tail` — which reports the
exit status of `tail`, not of the gate, and discarded all but the last 60 lines.
Recorded honestly, at the merged SHA and unpiped:

| field | value |
|---|---|
| command | `bash .claude/test-gate.sh > gate-full.log 2>&1; echo "GATE_EXIT=$?"` |
| SHA | `86907bb4af4e046fd8a5e829130d1fabdeb96ca9` (branch `phase1b`) |
| exit | `GATE_EXIT=0` |
| Rust | 655 passed, 0 failed (summed over every `test result:` line) |
| `@warden/core` | 301 passed (13 files) |
| `@warden/ui-tokens` | 11 passed |
| `spikes/03-txbudget` | 8 passed |

Every future green claim in this file carries its command, SHA and true exit
code, never a piped exit status.

## B4 — narrow invariant-ledger promotion (2026-08-23)

B4 was reassessed on 2026-08-22 as **not a batch flip**: promoting all 9 stale
`unimplemented` rows at once would have repeated the exact over-claim pattern
the two-model loop keeps catching. This pass therefore verified each row
independently against the test suite (evidence = test fn names, enforcement =
`file:line`) before touching its status.

| invariant | verdict | outcome |
|---|---|---|
| WRD-EXEC-01 | already fully covered | promoted `test-covered` |
| WRD-EXEC-05 | already fully covered | promoted `test-covered` |
| WRD-EXEC-08 | already fully covered | promoted `test-covered` |
| WRD-EXEC-02 | gap: no multi-ATA-same-mint case at any level | test added → promoted |
| WRD-DENY-01 | gap: Token-2022 never exercised (SPL only) | test parametrised → promoted |
| WRD-DENY-02 | gap: **zero** coverage | test + control added → promoted |
| WRD-STAGE-02 | gap: no stale generation / policy_version case | 2 tests added → promoted |
| WRD-CAP-09 | gap: no bucket-debit assertion; both over-threshold negatives were session-path | 3 tests added → promoted |
| WRD-FRZ-03 | compound over the unbuilt `execute_pending` | **stays `unimplemented`** |

Ledger: 48 → 56 `test-covered`, 39 → 31 `unimplemented`. `INVARIANTS.md`
regenerated by `scripts/gen-invariants.mjs` (88 rows).

**Two prior claims corrected by this pass.** (1) The 2026-08-22 note that
WRD-EXEC-08 "has no covering test found" was WRONG:
`runtime_program_owner_change_is_rejected`
(`programs/warden/src/conservation/tests.rs`) is precisely its SWIG-ACC-C1
"transfer out → close → reopen under a fake-layout program" case. (2) The same
note's read of WRD-DENY-01 as "only partial" was right but for an unstated
reason — the missing half was Token-2022, not the instruction set.

**Caveats recorded in each row's `notes` rather than glossed.** The
owner/mint field terms in `compare.rs` are defence-in-depth: in the tests that
cover them the reject actually fires one rule earlier (after-token-`None`),
because `snapshot_one` only decodes token fields for SPL/T22 owners, so a
case where *only* `owner_program` differs and both sides still parse is
unreachable. On swap, `swap_policy` sets `caps.per_tx == large_threshold.per_tx`,
so the pre-CPI bucket probe refuses `max_in` before the `large_threshold` rung
is reached — the execute counterpart does exercise `large_threshold` directly.
WRD-FRZ-03's swap half had no frozen test at all before this pass; `swap_frozen_rejected`
closes that hole without promoting the row.

**Anti-vacuity control.** The WRD-DENY-02 test plants a registry entry for
`(SPL_TOKEN_ID, SetAuthority)` in list 1 and asserts `DENY_LISTED` — *not*
`RegistryDenied` — which is what proves the decoder runs before the registry
