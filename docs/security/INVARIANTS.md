...
| `WRD-ORG-01` | **Origin/build-ID lifecycle fails closed** — Account creation binds the passkey to the exact production extension origin: an assertion whose clientDataJSON origin differs from the account's stored rp_id_hash is rejected, and the permitted production origin is pinned in deployment config so a dev extension cannot create a funded mainnet account. cluster_tag is caller-attested and is NOT the boundary; the origin binding is. | `unimplemented` | docs/superpowers/plans/2026-08-19-warden-client-security-hardening.md#C1a | `TOB-SQUADS-7` | — |
