...
| `WRD-EXT-02` | **Privileged UI port requires proven extension identity** — An extension UI port is privileged only when browser-owned sender identity proves it is the extension; content scripts cannot impersonate it. | `unimplemented` | docs/superpowers/plans/2026-08-19-warden-client-security-hardening.md#C1 | — | — |
...
| `WRD-QTE-01` | **Swap quote-source independence** — Both the primary and the check quote are bound to one canonical QuoteIntent (approval id + exact message digest, account, cluster, input/output mint pubkeys and decimals, ExactIn/ExactOut mode, base-unit amount, slippage/fee convention); a >300 bps divergence on the normalized output quantity blocks unoverridably, a stale primary re-quote that changes the intent invalidates the approval, and a check quote sharing the primary's upstream renders as no independent check, never as agreement. | `unimplemented` | docs/superpowers/plans/2026-08-19-warden-client-security-hardening.md#C4b | — | — |
