...
_Generated by `scripts/gen-invariants.mjs` from `docs/security/invariants.jsonl` — 83 invariants. Do not edit this table by hand._
...
| `WRD-ORG-01` | **Origin/build-ID lifecycle fails closed** — A stored-origin mismatch fails closed (an account created under origin A rejects an assertion whose clientDataJSON origin is B), and a dev-profile build cannot create a funded mainnet account; the production extension id is either frozen (migration refused) or migrated only through an authenticated Phase 1C path. | `unimplemented` | docs/superpowers/plans/2026-08-19-warden-client-security-hardening.md#C1a | `TOB-SQUADS-7` | — |
...
| `WRD-QTE-01` | **Swap quote-source independence** — The swap quote-sanity check records source/upstream/freshness/route provenance for both quotes; a stale primary re-quotes (never silently reuses), a >3% divergence blocks (unoverridable by simulation/reputation), and a check quote sharing the primary's upstream is rendered as no independent check, never as agreement. | `unimplemented` | docs/superpowers/plans/2026-08-19-warden-client-security-hardening.md#C4b | — | — |
