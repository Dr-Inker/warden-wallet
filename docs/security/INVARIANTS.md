...
| `WRD-DEP-01` | **Upgrade authority is the derived Squads vault PDA** — The deployment gate pins the Warden and Squads program ids, the expected mainnet multisig pubkey and member-pubkey set, threshold, time-lock, and vault index in versioned reviewed config; fetches the pinned Warden Program account (BPF Upgradeable Loader owner, Program state), authenticates the canonical ProgramData it encodes, asserts the on-chain multisig IS that pinned identity with Multisig.configAuthority == Pubkey::default() (autonomous, not reconfigurable by a single key); derives its canonical vault PDA; and requires ProgramData.upgrade_authority_address to equal it. The pinned Squads program's own code is authenticated (Program->ProgramData chain + audited Squads code hash); Squads' upgrade governance is an accepted documented external trust assumption where the root terminates. | `unimplemented` | design.md#5.5 | — | — |
| `WRD-DEP-02` | **Adapter selectors match on-chain Registry at deploy** — The deployment gate re-derives every adapter selector from source (Anchor sighash from the IDL where one exists, per-program instruction tag otherwise) and diffs it against the on-chain Registry, rejecting missing, extra, wrong, or duplicate selectors and role-validator mismatches. | `unimplemented` | design.md#5.5 | — | — |
