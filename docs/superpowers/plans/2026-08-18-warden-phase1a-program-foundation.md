...
#[account] pub struct SmartAccount {
    ...
    pub rp_id_hash: [u8; 32],        // SHA-256(full extension origin) — set at create
    pub origin: [u8; 64], pub origin_len: u8,   // "chrome-extension://<32 chars>" fits; stored bytes compared exactly
    pub generation: u64,
    pub root_nonce: u64,
    pub policy: Policy,
...
// rules: amount ≤ cap.per_tx; roll day bucket if now ≥ day_start+DAY_SECS (spent_today=0, day_start = now - now%DAY_SECS);
// ring: day_number = now / DAY_SECS; advance ring_day_index → day_number zeroing skipped slots (min(30, gap)); slot = day_number % 30;
...
```
Genesis hash: LiteSVM/localnet differ from mainnet — read it as `sysvar::instructions`-independent constant? No: obtain via `solana_program::sysvar::slot_hashes`? Not available. **Ruling in plan:** store `genesis_hash` in the SmartAccount at creation from a client-supplied value that the *extension* fills from `getGenesisHash()`; cross-cluster replay is prevented because the account address itself embeds nothing cluster-specific... to keep the spec's intent, store `cluster_tag: [u8;32]` at creation and include it in the transcript. Document this in the code (spec §4 wording "genesis_hash" = stored cluster tag).
...
Root payload budget check: `GrantSessionArgs` with 8 caps ≈ 8×56 + 60 ≈ 510 B — over the 400-B guidance → **cap the number of caps per grant instruction to 4** (`MAX_CAPS_PER_GRANT = 4`; a second grant call upserts more) and assert in a test that a 4-cap grant tx serializes ≤ 1,232 B including the precompile ix.

- [ ] Tests (RED): `grant_ok_and_readback`, `grant_needs_fresh_nonce` (reuse ⇒ NonceMismatch), `grant_over_ceiling_rejected`, `grant_ops_over_ceiling_rejected`, `grant_expiry_too_long_rejected`, `grant_frozen_rejected` (freeze via Task 6's root freeze — order: implement freeze first inside this task's branch? No: write this test after Task 6 lands, marked in Task 6), `revoke_by_session_self_ok`, `revoke_by_root_ok`, `revoke_by_stranger_rejected`, `grant_tx_fits_1232_bytes_with_4_caps` (serialize the real tx and assert length).
...
```
Rules: `frozen == None`; **session path**: `session.account == smart_account.key()`, `session.generation_at_grant == generation`, `now < expiry_ts`, `ops_mask & OP_TRANSFER != 0`, `signer == session.pubkey`; cap lookup in `session.caps` for mint (missing ⇒ `CapExceeded`) AND account-wide `policy.caps` (missing ⇒ `CapExceeded`); `buckets::debit(account.buckets[i], policy.caps[i], amount, now)`; per-session lifetime `lifetime_spent + amount ≤ lifetime_cap`; **root path**: `verify_root_assertion` with action 0x05 over `(mint, destination, amount)`; `amount ≤ policy.large_threshold[mint].per_tx` else `CapExceeded` (→ Phase 1B `queue`), and **the same `buckets::debit`** against `policy.caps` (root is bounded like a session — spec §5.2.4). Destination for SPL must not be a vault-owned token account (`destination.owner != smart_account`) — spec §5.1. SOL transfer via `invoke_signed(system_instruction::transfer(vault, dest))`? The vault PDA is a data account (Anchor `#[account]`), so use direct lamport arithmetic: `**smart_account.to_account_info().try_borrow_mut_lamports()? -= amount` (checked_sub) and `dest += amount`, and require post-balance ≥ rent-exempt minimum for the account's data (`Rent::get()?.minimum_balance(LEN)`) else `CapExceeded`. SPL via `token::transfer` CPI with `signer_seeds = [ACCOUNT_SEED, owner_seed, [bump]]`; ATA creation is the client's job (outer payer), never the vault.

- [ ] Tests (RED): `session_sol_transfer_within_caps`, `session_sol_over_per_tx_rejected`, `session_day_cap_across_two_txs`, `two_sessions_share_account_day_cap`, `session_expired_rejected`, `session_wrong_generation_rejected` (bump generation via `set_account` for now), `session_without_transfer_op_rejected`, `session_spl_transfer_ok` (mint fixture; balances move), `spl_to_vault_owned_ata_rejected`, `sol_transfer_cannot_breach_rent_floor`, `root_transfer_within_threshold_debits_buckets` (then a session transfer that would exceed the day cap fails — proves shared buckets), `root_transfer_over_threshold_rejected`, `frozen_blocks_transfer`, `rolling_30d_cap_enforced_end_to_end` (advance `svm` clock with `warp_to_slot`/set Clock sysvar via `svm.set_sysvar`), plus **CU measurement**: print CU for session SOL, session SPL, root SOL; record in `docs/program/PHASE1A-MEASUREMENTS.md`.
