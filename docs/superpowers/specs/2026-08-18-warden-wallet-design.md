...
**Date:** 2026-08-18 · **Status:** rev 3 (after Codex review rounds 1–2, see §16) · **Product:** drinkerlabs · **Working name:** Warden (rename = find/replace)
...
| `freeze` | root or guardian | Root freeze: indefinite until root `unfreeze` (after timelock). Guardian freeze: `until = now + guardian_freeze_max`, one active at a time, per-guardian cooldown 7 d; **while a Recovery proposal is open, a guardian freeze does not expire** |
| `unfreeze` | root | Root freeze: after timelock. Guardian freeze: **only via a queued (timelocked) action, and never while a Recovery proposal is open** — the root cannot insta-lift a guardian freeze |
