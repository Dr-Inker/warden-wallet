...
# so $1 is the verifier script path (ignored) and the rest are the verifier args.
...
# It ENFORCES the verifier's real flag/value contract so a renamed/missing/reordered
# flag fails HERE instead of passing silently, and records the exact args of BOTH
# the parse-mode and the live invocation.
va=("${@:2}")   # drop the verifier script path
declare -A m; i=0
while [ $i -lt ${#va[@]} ]; do
  k="${va[$i]}"; v="${va[$((i+1))]:-}"
