...
Coordinator re-review (round 3) found the same rule violated elsewhere: Phoenix, Sanctum, and
Realms were graded **firm** while their top-level-signer cell (Realms: also touched by an
UNVERIFIED aside in the SIWS cell) read UNVERIFIED — contradicting this document's own rule that
a firm verdict requires no UNVERIFIED cell in any of the four deciding columns. Unlike the
Magic Eden/Solend case, these three *were* resolvable with more digging rather than moving to
provisional: (1) **Phoenix** — the actual account-meta array lives in a generated TS file
(`src/instructions/Swap.ts`) that a plain grep for `isSigner: true`/`Signer<'info>` in the wrong
files had missed; `accounts.trader` is the sole signer. (2) **Sanctum** — the repo we'd cloned
(`sanctum-lst-list`) only holds LST metadata, not instruction builders; cloned the actual
`sanctum-router-sdk` repo and found the shared "prefix" accounts mark only `user` as signer, with
all pool-specific "suffix" accounts non-signer. (3) **Realms** — `@solana/spl-governance` is an
external npm package, not vendored in governance-ui; cloned its source repo (`solana-labs/oyster`)
and found `withCastVote` actually has *two* `isSigner:true` slots (`governanceAuthority`,
`payer`) — but governance-ui's own call site sets both to the same `walletPubkey`, so it's one
wallet signature, not two independent signers, resolving the cell as "No" rather than leaving it
open. Applying the rule mechanically across every remaining firm row (not just the three named)
also caught Pump.fun's `buy`/`sell` extra-signer and durable-nonce cells, which were bare
"UNVERIFIED" despite the row being firm — filled from the same published IDL already cited for
its discriminators (`buy`/`sell` have one signer each; `create`'s second signer is an ephemeral
mint keypair; no nonce account appears in any of the three). No verdict word changed for any of
these four rows (Phoenix stays OK, Sanctum stays root-only, Realms stays root-only/unsupported,
Pump.fun stays unsupported) — only the evidence backing them firmed up — so the firm/provisional
tallies (9/6/1 firm, 4 provisional) are unchanged from round 2.
...
| 11 | **Pump.fun** | Yes — Pump.fun uses Privy for wallet login; Privy's "Login with Wallet" implements SIWS [11a]; corroborated by `signMessage`/`signIn` strings in pump.fun's own production JS bundle [11b] | No for `buy`/`sell` — the published IDL lists exactly one signer, `user`, for both; `create` (new token launch, not the common trading flow) has a second signer, `mint`, but that's a freshly generated local keypair for the new SPL mint being created, the same ephemeral-signer pattern used by Orca/Meteora/Raydium above, not a co-signer [11e] | No — not found; the published IDL's `buy`/`sell`/`create` account lists contain no nonce-related account [11e] | Bonding-curve program `6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P` — `create`, `buy`, `sell` [11c] | No — the published IDL's `buy` instruction has exactly one `isSigner:true`/`signer:true` entry, `user` [11d] | **unsupported** (SIWS required to log in) |
| 12 | **Phoenix** | No — not found in phoenix-sdk (TS or Rust) | Not found required | No — not found | Program `PhoeNiXZ8ByJGLkxNfZRnkUfjvmuYqLR89jjFHGqdXY` [12a] — `Swap`, `PlaceLimitOrder`, `CancelAllOrders`, `WithdrawFunds` | No — found this round: the generated TS instruction builder's `createSwapInstruction` account-meta array has exactly one `isSigner: true` entry, `accounts.trader`, among 9 [12b] | **OK** |
| 13 | **Sanctum** | No — not found in `sanctum-lst-list` (rust/ts) | Not found required | No — not found | Sanctum Router `stkitrT1Uoy18Dk1fTrgPw8W6MVzoCfYoAFT4MLsmhq` [13a]; Infinity pool `5ocnV1qiCgaQR8Jb8xWnVbApfaygJ8tNoZfgPwsgx9kx` (per Sanctum docs — direct WebFetch of learn.sanctum.so returned 403; cited via WebSearch's synopsis of that same URL, not independently re-fetched this round) [13b] — swap / addLiquidity / removeLiquidity | No — found this round in the separate `sanctum-router-sdk` repo (the actual account-meta source, not `sanctum-lst-list`): the shared "prefix" accounts used by deposit/swap instructions mark only `user` as signer (`STAKE_WRAPPED_SOL_PREFIX_IS_SIGNER` sets `.const_with_user(true)` and defaults everything else `false`), and the pool-specific "suffix" accounts (stake-pool program, pool, withdraw authority, fee/reserve accounts) are all non-signers (`SPL_DEPOSIT_SOL_IX_SUFFIX_IS_SIGNER` is all-`false`) [13c] | **root-only** |
...
| 18 | **Realms** (governance-ui + Hub) | **Mixed.** Core governance app: No — `spl-governance` voting/proposal calls found no signMessage. **Realms Hub** (forum/profile layer, same repo, separate sub-product): **Yes** — `verify-wallet/components/sign-in-with-solana.tsx` explicitly requests a server-issued claim, calls `signMessage(claimBlob)`, and posts the signature to mint a JWT — this is SIWS by name [18a] | Not found required for voting/proposal instructions | No — not found | `@solana/spl-governance` program `GovER5Lthms3bLBqWub97yVrMmEogzX7xNjdXpPPCVZw` (well-known; referenced via `DEFAULT_GOVERNANCE_PROGRAM_ID` import, not hardcoded in this repo) [18b] — `castVote`, `createProposal`, `depositGoverningTokens` | No — found this round by cloning `@solana/spl-governance`'s actual source repo (`solana-labs/oyster`, not vendored in governance-ui): `withCastVote`'s account-meta array has **two** `isSigner:true` entries, `governanceAuthority` and `payer` — but governance-ui's own `castVote.ts` sets both parameters to the same `walletPubkey` at the call site, so it's one distinct key (the wallet) satisfying two account slots, not two independent required signers [18c] | **root-only** for voting/proposals; **unsupported** for the Realms Hub SIWS profile-verification flow (see SIWS-login list) |
...
- **[11e]** same source, `buy`/`sell` accounts each carry exactly one `signer:true` entry (`user`); `create` carries two (`mint`, `user`) where `mint` is a fresh keypair for the new token being launched (ephemeral-signer pattern, not a co-signer); no nonce-related account name appears in any of the three instructions' account lists
...
- **[12b]** `src-phoenix/typescript/phoenix-sdk/src/instructions/Swap.ts:96-104` @ `d722fc16c33feadec99f7d7b01c3ac4e12e44e75` — `keys: web3.AccountMeta[]` for `createSwapInstruction`; `accounts.trader` is the only entry with `isSigner: true` (8 other entries are all `isSigner: false`)
...
- **[13c]** `src-sanctum-router/core/src/instructions/stake_wrapped_sol.rs:34-35` (`STAKE_WRAPPED_SOL_PREFIX_IS_SIGNER = ...const_with_user(true)`, all other prefix accounts default `false`) and `src-sanctum-router/core/src/routers/spl/deposit_sol.rs:93-94` (`SPL_DEPOSIT_SOL_IX_SUFFIX_IS_SIGNER = [false; ...]`, i.e. the pool-specific suffix accounts are all non-signers) @ `86b987abc1d179210a9a6bb26158341470add27a`, https://github.com/igneous-labs/sanctum-router-sdk
...
- **[18b]** `src-realms/pages/dao/[symbol]/proposal/components/instructions/WithdrawFromDAO.tsx:39` (imports `DEFAULT_GOVERNANCE_PROGRAM_ID` from `@solana/governance-program-library`) @ same sha; program id `GovER5Lthms3bLBqWub97yVrMmEogzX7xNjdXpPPCVZw` is the well-known public spl-governance deployment
- **[18c]** `src-realms/actions/castVote.ts:1-27` shows `withCastVote` imported from `@solana/spl-governance`; cloned that package's actual source repo this round (`npm view @solana/spl-governance repository` → `solana-labs/oyster`) — `src-oyster/packages/governance-sdk/src/governance/withCastVote.ts:49-96` @ `6505a1624f653dd4e4a431f405a6895597fd7133`, https://github.com/solana-labs/oyster, shows `governanceAuthority` (line ~78) and `payer` (line ~93) both marked `isSigner: true`; `src-realms/actions/castVote.ts:191-192` @ `531440354a44259e038dc105f0ae6f604cc229cb` shows `const governanceAuthority = walletPubkey; const payer = walletPubkey` — both slots resolve to the same connected-wallet address in governance-ui's own usage
...
5. SPL Stake Pool's (Jito) exact account-meta signer list was not independently verified —
   we cloned the Jito bundle/searcher client (`jito-labs/jito-ts`), not `spl-stake-pool` itself
   — which is why that row stayed provisional. (Round 3: the equivalent gap for Realms —
   `@solana/spl-governance` being an external, non-vendored package — was closed by cloning its
   actual source repo, `solana-labs/oyster`; Realms' top-level-signer cell is no longer
   UNVERIFIED, see [18c].)
